                    ...
                    <div class="row justify-content-center">   
                      ...
                      function showPosition(position) {
                        ...
                        $('#longitude_pegawai').val(position.coords.longitude);
                          ...
                          <select class="form-select form-select-sm mb-3" aria-label="Large select example"  id="lokasi" name="lokasi">
                              <option value="0">Lokasi Dinas</option>
                              <?php
                              $kantor = mysqli_query($koneksi,"select * from kantor order by nama_kantor asc");  
                              while($row=mysqli_fetch_array($kantor)){  ?>
                              <?php
                              if($jab ==$row['nama_kantor']){
                                echo'<option value="'.$row['id_kantor'].'" selected>'.$row['nama_kantor'].'</option>';
                              }else{
                          ...
                          <select class="form-select form-select-sm mb-3" aria-label="Large select example" name="shift">
                            <option value="0">Shift</option>
                            <?php
                            $shift = mysqli_query($koneksi,"select * from shift_kerja order by nama_shift asc");  
                            while($row=mysqli_fetch_array($shift)){  ?>
                            <?php
                            if($jab ==$row['nama_shift']){
                              echo'<option value="'.$row['id_shift'].'" selected>'.$row['nama_shift'].'</option>';
                            }else{
                              echo'<option value="'.$row['id_shift'].'">'.$row['nama_shift'].'</option>';}
                            ?>
                            <!-- <option value="<?php echo $row['id_shift']?>"><?php echo $row['nama_shift']?></option> -->
                            <?php  } ?>                      
                            </select> 
