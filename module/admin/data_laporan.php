...
if(isset($_POST['jabatan']) and $_POST['jabatan'] != "*"){
  ...
  $data_lapbul= mysqli_query($koneksi,"select * from lap_bulanan lb join user us on lb.id_peg = us.id where jabatan !='admin' and jabatan !='sudimin' and jabatan !='Magang' and bulan = '$bulan' and tahun ='$tahun' $filteruser order by nama_pegawai asc");	 	
  $data_nonlb= mysqli_query($koneksi,"select * from user where jabatan !='admin' and jabatan !='sudimin' and jabatan !='Magang' and id NOT IN (SELECT id_peg FROM lap_bulanan where bulan='$bulan' and tahun='$tahun') $filteruser order by nama_pegawai asc");	
