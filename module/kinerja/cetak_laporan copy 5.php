     ...
     <?php     
      ...
      ?>
        ...
        <tbody>
              ...
              <?php             
            $tahun = date('Y');
            $tgllap = mysqli_query($koneksi,"select distinct tgl_lap from data_laporan dl join user u on dl.id_peg = u.id where bulan ='$angka' and tahun ='$tahun' and id_peg ='$user_id'");  
            while($tgl=mysqli_fetch_array($tgllap)){ 
                $hasiltgl = $tgl['tgl_lap'];   
            $dataLap = mysqli_query($koneksi,"select * from data_laporan dl join user u on dl.id_peg = u.id where bulan ='$angka' and tahun ='$tahun' and id_peg ='$user_id'");  
            $no=0;
              ?>
              
              <tr>
                <td rowspan="4">bingung</td>
              </tr>
              <?php
